...
2b. Quick Local Setup with SQLite (no PostgreSQL)

If you want to run the application locally without setting up PostgreSQL, you can take advantage of SQLite, which is built into Python. This option is perfect for quick testing or development because it requires zero database configuration— the finance.db file will be created automatically on first run and all tables are generated by the application.

To use SQLite instead of PostgreSQL:

Do not set the DATABASE_URL environment variable in your .env file. By default, the app falls back to SQLite and uses sqlite:///finance.db.

Make sure your virtual environment is activated and dependencies are installed (see step 1 in the existing Local Development Setup section).
...
The first time you run the app, a new file called finance.db will appear in your project directory. This is your local SQLite database. The application will automatically create all required tables the first time it starts.

Open your browser to http://localhost:5000
. You can register a user and start adding transactions immediately. Your data will be saved in finance.db. If you ever want to reset your local data, simply delete the finance.db file and it will be recreated on the next run.
```
